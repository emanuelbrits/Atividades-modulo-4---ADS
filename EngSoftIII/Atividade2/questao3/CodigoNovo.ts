// métodos alterados: estaValido no produto perecível e inserir no estoque
...
class ProdutoPerecivel extends Produto {
    ...
    public estaValido(dtAtual: string): boolean{
        ...
        if((dtVenc[2] < dtAtualn[2]) || (dtVenc[1] < dtAtualn[1] && dtVenc[2] <= dtAtualn[2])) {
            throw new console.error('Produto passou da validade');
        }

        if((dtVenc[0] < dtAtualn[0] && dtVenc[1] <= dtAtualn[1] && dtVenc[2] <= dtAtualn[2])) {
            throw new console.error('Produto passou da validade');
        }
...
class Estoque{
    ...
    public inserir(produto: Produto):void {
        if(this.verificar(produto._id)) {
            throw new console.error("Produto já registrado");
        }
